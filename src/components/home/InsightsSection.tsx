      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div ref={sectionAnim.ref} className={cn("transition-all duration-700", sectionAnim.isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8")}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {articles.map((article, index) => <article key={index} className={cn("group bg-card border border-border rounded-xl overflow-hidden hover:border-accent/50 transition-all duration-500 hover:shadow-lg", isRTL && "font-hebrew text-right", sectionAnim.isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8")} style={{
            transitionDelay: sectionAnim.isVisible ? `${index * 150}ms` : '0ms'
          }}>
                {/* Image Container */}
                <div className="relative h-48 overflow-hidden">
                  <img src={article.image} alt={t(article.titleKey)} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" />
                ...
                <div className="p-6">
                  ...
                  <Link to="/insights" className={cn("inline-flex items-center gap-2 text-accent font-medium text-sm hover:gap-3 transition-all", isRTL && "flex-row-reverse")}>
                    {t('insights.readMore')}
                    <Arrow className="h-4 w-4" />
                  </Link>
                </div>
